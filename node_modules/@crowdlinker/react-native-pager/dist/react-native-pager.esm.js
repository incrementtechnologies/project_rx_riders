import React, { useContext, useState, Children, useEffect, createContext, useRef } from 'react';
...
function Pager(_ref) {
  ...
  var animatedIndex = memoize(block([cond(eq(gestureState, State.ACTIVE), [cond(clockRunning(clock), stopClock(clock)), // captures the initial drag value on first drag event
  ...
  function renderChildren() {
    ...
    return adjacentChildren.map(function (child, i) {
      ...
      return React.createElement(IndexProvider, {
        index: index,
        key: index
      }, React.createElement(FocusProvider, {
  ...
  }), React.createElement(PanGestureHandler, Object.assign({}, panProps, {
...
function PagerProvider(_ref7) {
  ...
  var _useState4 = useState(initialIndex),
  ...
  return React.createElement(PagerContext.Provider, {
...
function useFocus() {
  var focused = useContext(FocusContext);
  return focused;
}
...
export { Extrapolate, FocusProvider, IndexProvider, Pager, PagerContext, PagerProvider, Pagination, Progress, Slider, interpolateWithConfig$1 as interpolateWithConfig, useAnimatedIndex, useFocus, useIndex, useInterpolation, useOffset, useOnFocus, usePager };
